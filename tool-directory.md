...
| `list_schematic_texts` | Read plain annotations from the saved schematic, returning UUIDs, positions in millimetres and a document revision. Excludes net labels, fields, text boxes and nested library text. |
| `edit_schematic_text` | Replace an annotation's text by UUID and exact saved revision. Preserves formatting and unrelated objects, refuses editor-owned files and verifies committed readback. |
| `delete_schematic_text` | Delete an annotation by UUID and exact saved revision, refusing group members and editor-owned files. Verifies that the selected UUID is absent afterward. |
