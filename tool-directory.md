...
| `set_design_rules` | Set board-level clearance, trace, via diameter/drill, annular-ring width and hole-to-copper/hole-to-hole limits in the sibling `.kicad_pro`. Omitted limits are preserved; save and close the project before writing, then reopen to load changes. The board file is not modified. |
